...
void lexer_parse_word(Lexer *lexer) {
  ...
  if (strcmp(buffer, "if") == 0)
    tokenlist_add(lexer->tokens,
                  make_token(TOKEN_IF, imp_arena_strdup(arena, buffer), span));
  else if (strcmp(buffer, "for") == 0)
    tokenlist_add(lexer->tokens,
                  make_token(TOKEN_FOR, imp_arena_strdup(arena, buffer), span));
  else
    tokenlist_add(
        lexer->tokens,
        make_token(TOKEN_IDENTIFIER, imp_arena_strdup(arena, buffer), span));
}
void lexer_parse_string(Lexer *lexer) {
  ...
  tokenlist_add(lexer->tokens,
                make_token(TOKEN_STRING, imp_arena_strdup(arena, buffer),
                           make_span(line, sc, lexer->col - 1)));
}
